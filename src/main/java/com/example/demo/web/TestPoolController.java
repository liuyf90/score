...
@RestController
@RequestMapping("/test")
public class TestPoolController extends BaseController{
    @Autowired
...
//        status.put(0,"未领取");
    ...
    public ModelAndView init(@PathVariable(value = "id", required = false) Long id, Model model) {
       List<Task> ll= taskservice.findByFinish(TaskStatus.CHECK);
        for(Task task:ll){
            for(TaskUser tuser:task.getReceivers()){
               if(tuser.getRoleType()==0){
                   task.setReceiverName(tuser.getUser().getCname());
               }
            }
        }

        model.addAttribute("taskList", ll);
